<script>
  import { isAcc } from "../Stores/dataPoints.js";
  import { currentAcc } from "../Stores/acceleration.js";
  import { scaleLinear } from "d3-scale";
  /**
   * Komponente des ACI-Displays (kWh/km)
   */

  /**
   * Skala besimmt die Position Balken im Display.
   * domain: Werteintervall
   * range: y-Werte des Displays
   */
  const yScale = scaleLinear().domain([0, 2]).range([614.456, 82.776]);

  //Array enthält 4 Beschleunigungen als Objekte
  let accels = [{ average: 0 }, { average: 0 }, { average: 0 }, { average: 0 }];
  //Aktuelle Beschleunigung
  let currentAccel = {};
  let toggle = false;
  let isBeschl;

  /**
   * Abonniert den Store "isAcc". Wird aufgerufen wenn sich Wert des Stores ändert.
   * Weise dann den aktuellen Wert der Variable "isBeschl" zu.
   */
  isAcc.subscribe((newValue) => {
    isBeschl = newValue;
  });

  /**
   * Abonniert den Store "currentAcc". Wird aufgerufen wenn sich Wert des Stores ändert.
   * Fügt die jüngste Beschleunigung zum Array "Accels" hinzu und löscht die älteste Beschleunigung aus dem Array.
   * Dabei werden nur Beschleunigungen hinzugefügt die länger als 1 Sekunde dauern.
   */
  currentAcc.subscribe((newValue) => {
    if (
      isBeschl == false &&
      newValue.over == true &&
      newValue.duration > 1 &&
      toggle == false
    ) {
      accels[3] = accels[2];
      accels[2] = accels[1];
      accels[1] = accels[0];
      if (newValue.average == undefined) {
        accels[0] = 0;
      } else {
        accels[0] = newValue;
      }
      toggle = true;
    } else if (isBeschl == true && newValue.over == false) {
      currentAccel = newValue;
      toggle = false;
    }
  });
</script>

<!--
  ---DISPLAY---
  Code für Animation: Zeile 123 - 220
-->
<svg
  width="100%"
  height="100%"
  viewBox="0 0 704 709"
  version="1.1"
  xmlns="http://www.w3.org/2000/svg"
  xmlns:xlink="http://www.w3.org/1999/xlink"
  xml:space="preserve"
  xmlns:serif="http://www.serif.com/"
  style="fill-rule:evenodd;clip-rule:evenodd;stroke-linecap:round;stroke-miterlimit:1.5;"
>
  <g id="ACI_mehr">
    <use
      id="Hintergrund"
      xlink:href="#_Image1"
      x="8.847"
      y="8.036"
      width="685.76px"
      height="691.982px"
      transform="matrix(0.99965,0,0,0.999974,0,0)"
    />
    <path
      id="Hintergrund1"
      serif:id="Hintergrund"
      d="M656.696,90.032c0,-23.783 -19.308,-43.092 -43.092,-43.092l-522.76,0c-23.784,0 -43.093,19.309 -43.093,43.092l0,527.988c0,23.783 19.309,43.092 43.093,43.092l522.76,0c23.784,0 43.092,-19.309 43.092,-43.092l0,-527.988Z"
      style="fill:url(#_Linear2);"
    />
    <g>
      <use
        id="Display_Area"
        xlink:href="#_Image3"
        x="62.704"
        y="61.963"
        width="421.368px"
        height="571.634px"
        transform="matrix(0.998503,0,0,0.99936,0,0)"
      />
      <rect
        id="Display_Area1"
        serif:id="Display_Area"
        x="80.849"
        y="81.161"
        width="384.889"
        height="533.158"
        style="fill:rgb(15,18,21);"
      />
      <use
        id="Display_Area_ICI"
        xlink:href="#_Image4"
        x="452.057"
        y="61.963"
        width="133.042px"
        height="571.634px"
        transform="matrix(0.992853,0,0,0.99936,0,0)"
      />
      <rect
        id="Display_Area_ICI1"
        serif:id="Display_Area_ICI"
        x="467.071"
        y="81.161"
        width="96.552"
        height="533.158"
        style="fill:rgb(15,18,21);"
      />
      <g id="ACI0">
        <rect
          x="371.262"
          y={yScale(accels[0].average)}
          width="93.23"
          height={614.456 - yScale(accels[0].average)}
          style="fill:rgb({accels[0].r},{accels[0].g},{accels[0]
            .b});fill-opacity:0.48;"
        />
        {#if accels[0] !== undefined && accels[0].average > 0}
          <rect
            x="371.262"
            y={yScale(accels[0].average)}
            width="93.262"
            height="6.94"
            style="fill:rgb({accels[0].r},{accels[0].g},{accels[0].b});"
          />
        {/if}
        <g id="Achsen">
          <path
            d="M466.297,81.403l-0,533.512l0.094,-0l-0,-533.512l-0.094,-0Z"
            style="fill:none;stroke:url(#_Linear6);stroke-width:3px;stroke-linejoin:round;"
          />
        </g>
      </g>
      <g id="ACI1" serif:id="ICI">
        <rect
          x="274.483"
          y={yScale(accels[1].average)}
          width="93.23"
          height={614.456 - yScale(accels[1].average)}
          style="fill:rgb({accels[1].r},{accels[1].g},{accels[1]
            .b});fill-opacity:0.48;"
        />
        {#if accels[1] !== undefined && accels[1].average > 0}
          <rect
            x="274.483"
            y={yScale(accels[1].average)}
            width="93.262"
            height="6.94"
            style="fill:rgb({accels[1].r},{accels[1].g},{accels[1].b});"
          />
        {/if}
      </g>
      <g id="ACI2" serif:id="ICI">
        <rect
          x="177.747"
          y={yScale(accels[2].average)}
          width="93.23"
          height={614.456 - yScale(accels[2].average)}
          style="fill:rgb({accels[2].r},{accels[2].g},{accels[2]
            .b});fill-opacity:0.48;"
        />
        {#if accels[2] !== undefined && accels[2].average > 0}<rect
            x="177.747"
            y={yScale(accels[2].average)}
            width="93.262"
            height="6.94"
            style="fill:rgb({accels[2].r},{accels[2].g},{accels[2].b});"
          />
        {/if}
      </g>
      <g id="ACI3" serif:id="ICI">
        <rect
          x="80.898"
          y={yScale(accels[3].average)}
          width="93.23"
          height={614.456 - yScale(accels[3].average)}
          style="fill:rgb({accels[3].r},{accels[3].g},{accels[3]
            .b});fill-opacity:0.48;"
        />
        {#if accels[3] !== undefined && accels[3].average > 0}
          <rect
            x="80.898"
            y={yScale(accels[3].average)}
            width="93.262"
            height="6.94"
            style="fill:rgb({accels[3].r},{accels[3].g},{accels[3].b});"
          />
        {/if}
      </g>
      <g id="ICI" serif:id="ICI">
        {#if isBeschl == true && currentAccel.average > 0}
          <rect
            x="467.566"
            y={yScale(currentAccel.average)}
            width="93.23"
            height={614.456 - yScale(currentAccel.average)}
            style="fill:rgb({currentAccel.r},{currentAccel.g},{currentAccel.b});
                fill-opacity:0.48"
          />
          <rect
            x="467.566"
            y={yScale(currentAccel.average)}
            width="97.352"
            height="6.94"
            style="fill:rgb({currentAccel.r},{currentAccel.g},{currentAccel.b});"
          />
        {/if}

        <g id="Achsen4" serif:id="Achsen">
          <path
            d="M562.601,81.403l0,533.512l0.094,-0l0,-533.512l-0.094,-0Z"
            style="fill:none;stroke:url(#_Linear8);stroke-width:3px;stroke-linejoin:round;"
          />
        </g>
      </g>
      <g id="ICI5" serif:id="ICI" />
      <g id="Skalierung-und-Achsen" serif:id="Skalierung und Achsen">
        <g id="Achsen5" serif:id="Achsen">
          <path
            d="M464.686,81.195l0,533.261l1,-0l0,-533.261l-1,0Z"
            style="fill:none;stroke:url(#_Linear9);stroke-width:3px;stroke-linejoin:round;"
          />
          <rect
            x="80.818"
            y="614.456"
            width="481.752"
            height="1"
            style="fill:none;stroke:url(#_Linear10);stroke-width:3px;stroke-linejoin:round;"
          />
        </g>
        <g id="Skalierung">
          <rect
            x="553.147"
            y="505.856"
            width="10"
            height="1"
            style="fill:none;stroke:url(#_Linear11);stroke-width:3px;stroke-linejoin:round;"
          />
          <rect
            x="553.147"
            y="398.256"
            width="10"
            height="1"
            style="fill:none;stroke:url(#_Linear12);stroke-width:3px;stroke-linejoin:round;"
          />
          <rect
            x="553.147"
            y="290.656"
            width="10"
            height="1"
            style="fill:none;stroke:url(#_Linear13);stroke-width:3px;stroke-linejoin:round;"
          />
          <rect
            x="553.147"
            y="183.056"
            width="10"
            height="1"
            style="fill:none;stroke:url(#_Linear14);stroke-width:3px;stroke-linejoin:round;"
          />
          <rect
            x="553.135"
            y="82.776"
            width="10"
            height="1"
            style="fill:none;stroke:url(#_Linear15);stroke-width:3px;stroke-linejoin:round;"
          />
        </g>
        <g id="Skalierung_Beschriftung">
          <text
            x="566.655px"
            y="514.288px"
            style="font-family:'Roboto-Medium', 'Roboto';font-weight:500;font-size:20px;fill:white;"
            >0.4</text
          >
          <text
            x="566.655px"
            y="406.688px"
            style="font-family:'Roboto-Medium', 'Roboto';font-weight:500;font-size:20px;fill:white;"
            >0.8</text
          >
          <text
            x="566.655px"
            y="299.877px"
            style="font-family:'Roboto-Medium', 'Roboto';font-weight:500;font-size:20px;fill:white;"
            >1.2</text
          >
          <text
            x="566.655px"
            y="191.488px"
            style="font-family:'Roboto-Medium', 'Roboto';font-weight:500;font-size:20px;fill:white;"
            >1.6</text
          >
          <text
            x="566.655px"
            y="91.188px"
            style="font-family:'Roboto-Medium', 'Roboto';font-weight:500;font-size:20px;fill:white;"
            >2.0</text
          >
        </g>
      </g>
      <g id="Graphlines">
        <rect
          id="_30"
          serif:id="30"
          x="80.947"
          y="398.301"
          width="480.918"
          height="1"
          style="fill:none;stroke:url(#_Linear16);stroke-width:1px;stroke-linejoin:round;"
        />
        <rect
          id="_301"
          serif:id="30"
          x="80.947"
          y="183.101"
          width="480.918"
          height="1"
          style="fill:none;stroke:url(#_Linear17);stroke-width:1px;stroke-linejoin:round;"
        />
        <rect
          id="_302"
          serif:id="30"
          x="80.947"
          y="290.701"
          width="480.918"
          height="1"
          style="fill:none;stroke:url(#_Linear18);stroke-width:1px;stroke-linejoin:round;"
        />
        <rect
          id="_303"
          serif:id="30"
          x="80.947"
          y="505.896"
          width="480.918"
          height="1"
          style="fill:none;stroke:url(#_Linear19);stroke-width:1px;stroke-linejoin:round;"
        />
        <rect
          id="_304"
          serif:id="30"
          x="80.947"
          y="82.801"
          width="480.918"
          height="1"
          style="fill:none;stroke:url(#_Linear20);stroke-width:1px;stroke-linejoin:round;"
        />
      </g>
    </g>
    <text
      x="566.9px"
      y="615.935px"
      style="font-family:'Roboto-Medium', 'Roboto';font-weight:500;font-size:20px;fill:white;"
      >kWh/km</text
    >
  </g>
  <defs>
    <image
      id="_Image1"
      width="686px"
      height="692px"
      xlink:href="data:image/png;base64,"
    />
    <linearGradient
      id="_Linear2"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(608.945,0,0,614.172,47.7514,354.026)"
      ><stop offset="0" style="stop-color:rgb(35,41,47);stop-opacity:1" /><stop
        offset="0.62"
        style="stop-color:rgb(40,48,55);stop-opacity:1"
      /><stop
        offset="0.82"
        style="stop-color:rgb(53,66,74);stop-opacity:1"
      /><stop
        offset="1"
        style="stop-color:rgb(68,85,95);stop-opacity:1"
      /></linearGradient
    >
    <image
      id="_Image3"
      width="422px"
      height="572px"
      xlink:href="data:image/png;base64,"
    />
    <image
      id="_Image4"
      width="134px"
      height="572px"
      xlink:href="data:image/png;base64,"
    />
    <image
      id="_Image5"
      width="122px"
      height="36px"
      xlink:href="data:image/png;base64,"
    />
    <linearGradient
      id="_Linear6"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(3.16179e-15,-533.512,0.0938807,6.12443e-17,466.344,614.915)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.59"
        style="stop-color:rgb(253,255,255);stop-opacity:0.95"
      /><stop
        offset="0.8"
        style="stop-color:rgb(247,254,255);stop-opacity:0.84"
      /><stop
        offset="1"
        style="stop-color:rgb(242,253,255);stop-opacity:0.74"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <image
      id="_Image7"
      width="126px"
      height="36px"
      xlink:href="data:image/png;base64,"
    />
    <linearGradient
      id="_Linear8"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(3.16179e-15,-533.512,0.0938807,6.12443e-17,562.648,614.915)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.59"
        style="stop-color:rgb(253,255,255);stop-opacity:0.95"
      /><stop
        offset="0.8"
        style="stop-color:rgb(247,254,255);stop-opacity:0.84"
      /><stop
        offset="1"
        style="stop-color:rgb(242,253,255);stop-opacity:0.74"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear9"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(3.26528e-14,-533.26,1,6.12323e-17,465.186,614.456)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.59"
        style="stop-color:rgb(253,255,255);stop-opacity:0.95"
      /><stop
        offset="0.8"
        style="stop-color:rgb(247,254,255);stop-opacity:0.84"
      /><stop
        offset="1"
        style="stop-color:rgb(242,253,255);stop-opacity:0.74"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear10"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(-481.752,-5.89976e-14,1.22465e-16,-1,562.569,614.956)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.64"
        style="stop-color:rgb(252,255,255);stop-opacity:0.95"
      /><stop
        offset="0.84"
        style="stop-color:rgb(246,254,255);stop-opacity:0.82"
      /><stop
        offset="1"
        style="stop-color:rgb(238,252,255);stop-opacity:0.66"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear11"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(10,0,0,1,553.147,506.356)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.53"
        style="stop-color:rgb(252,255,255);stop-opacity:0.95"
      /><stop
        offset="0.69"
        style="stop-color:rgb(246,254,255);stop-opacity:0.82"
      /><stop
        offset="0.82"
        style="stop-color:rgb(238,252,255);stop-opacity:0.66"
      /><stop
        offset="0.93"
        style="stop-color:rgb(232,251,255);stop-opacity:0.55"
      /><stop
        offset="0.97"
        style="stop-color:rgb(218,249,255);stop-opacity:0.28"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear12"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(10,0,0,1,553.147,398.756)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.53"
        style="stop-color:rgb(252,255,255);stop-opacity:0.95"
      /><stop
        offset="0.69"
        style="stop-color:rgb(246,254,255);stop-opacity:0.82"
      /><stop
        offset="0.82"
        style="stop-color:rgb(238,252,255);stop-opacity:0.66"
      /><stop
        offset="0.93"
        style="stop-color:rgb(232,251,255);stop-opacity:0.55"
      /><stop
        offset="0.97"
        style="stop-color:rgb(218,249,255);stop-opacity:0.28"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear13"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(10,0,0,1,553.147,291.156)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.53"
        style="stop-color:rgb(252,255,255);stop-opacity:0.95"
      /><stop
        offset="0.69"
        style="stop-color:rgb(246,254,255);stop-opacity:0.82"
      /><stop
        offset="0.82"
        style="stop-color:rgb(238,252,255);stop-opacity:0.66"
      /><stop
        offset="0.93"
        style="stop-color:rgb(232,251,255);stop-opacity:0.55"
      /><stop
        offset="0.97"
        style="stop-color:rgb(218,249,255);stop-opacity:0.28"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear14"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(10,0,0,1,553.147,183.556)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.53"
        style="stop-color:rgb(252,255,255);stop-opacity:0.95"
      /><stop
        offset="0.69"
        style="stop-color:rgb(246,254,255);stop-opacity:0.82"
      /><stop
        offset="0.82"
        style="stop-color:rgb(238,252,255);stop-opacity:0.66"
      /><stop
        offset="0.93"
        style="stop-color:rgb(232,251,255);stop-opacity:0.55"
      /><stop
        offset="0.97"
        style="stop-color:rgb(218,249,255);stop-opacity:0.28"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear15"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(10,0,0,1,553.135,83.2758)"
      ><stop offset="0" style="stop-color:white;stop-opacity:1" /><stop
        offset="0.53"
        style="stop-color:rgb(252,255,255);stop-opacity:0.95"
      /><stop
        offset="0.69"
        style="stop-color:rgb(246,254,255);stop-opacity:0.82"
      /><stop
        offset="0.82"
        style="stop-color:rgb(238,252,255);stop-opacity:0.66"
      /><stop
        offset="0.93"
        style="stop-color:rgb(232,251,255);stop-opacity:0.55"
      /><stop
        offset="0.97"
        style="stop-color:rgb(218,249,255);stop-opacity:0.28"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear16"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(-480.918,-5.88955e-14,1.22465e-16,-1,561.865,398.801)"
      ><stop offset="0" style="stop-color:white;stop-opacity:0.25" /><stop
        offset="0.12"
        style="stop-color:rgb(242,253,255);stop-opacity:0.45"
      /><stop
        offset="0.27"
        style="stop-color:rgb(238,252,255);stop-opacity:0.5"
      /><stop
        offset="0.62"
        style="stop-color:rgb(229,250,255);stop-opacity:0.37"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear17"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(-480.918,-5.88955e-14,1.22465e-16,-1,561.865,183.601)"
      ><stop offset="0" style="stop-color:white;stop-opacity:0.25" /><stop
        offset="0.12"
        style="stop-color:rgb(242,253,255);stop-opacity:0.45"
      /><stop
        offset="0.27"
        style="stop-color:rgb(238,252,255);stop-opacity:0.5"
      /><stop
        offset="0.62"
        style="stop-color:rgb(229,250,255);stop-opacity:0.37"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear18"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(-480.918,-5.88955e-14,1.22465e-16,-1,561.865,291.201)"
      ><stop offset="0" style="stop-color:white;stop-opacity:0.25" /><stop
        offset="0.12"
        style="stop-color:rgb(242,253,255);stop-opacity:0.45"
      /><stop
        offset="0.27"
        style="stop-color:rgb(238,252,255);stop-opacity:0.5"
      /><stop
        offset="0.62"
        style="stop-color:rgb(229,250,255);stop-opacity:0.37"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear19"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(-480.918,-5.88955e-14,1.22465e-16,-1,561.865,506.396)"
      ><stop offset="0" style="stop-color:white;stop-opacity:0.25" /><stop
        offset="0.12"
        style="stop-color:rgb(242,253,255);stop-opacity:0.45"
      /><stop
        offset="0.27"
        style="stop-color:rgb(238,252,255);stop-opacity:0.5"
      /><stop
        offset="0.62"
        style="stop-color:rgb(229,250,255);stop-opacity:0.37"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
    <linearGradient
      id="_Linear20"
      x1="0"
      y1="0"
      x2="1"
      y2="0"
      gradientUnits="userSpaceOnUse"
      gradientTransform="matrix(-480.918,-5.88955e-14,1.22465e-16,-1,561.865,83.301)"
      ><stop offset="0" style="stop-color:white;stop-opacity:0.25" /><stop
        offset="0.12"
        style="stop-color:rgb(242,253,255);stop-opacity:0.45"
      /><stop
        offset="0.27"
        style="stop-color:rgb(238,252,255);stop-opacity:0.5"
      /><stop
        offset="0.62"
        style="stop-color:rgb(229,250,255);stop-opacity:0.37"
      /><stop
        offset="1"
        style="stop-color:rgb(204,246,255);stop-opacity:0"
      /></linearGradient
    >
  </defs>
</svg>

<style>
  svg {
    width: 50%;
    height: auto;
    display: block;
    margin: auto;
  }
</style>
