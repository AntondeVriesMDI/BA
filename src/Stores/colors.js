 ...
 */
 export const currentColor = derived( [currentDataPoint, currentEce] , ([$currentDataPoint,$currentEce]) =>{
...
export const getColor = function(percDist){
    ...
    const rScale = scaleLinear().domain([domainstart.percent, domainend.percent]).range([domainstart.r, domainend.r]);
    const gScale = scaleLinear().domain([domainstart.percent, domainend.percent]).range([domainstart.g, domainend.g]);
    const bScale = scaleLinear().domain([domainstart.percent, domainend.percent]).range([domainstart.b, domainend.b]);
